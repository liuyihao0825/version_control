...
int commit_git_repo()
{
    ...
    */
    ...
    const char *entries[] = { "readme" };
    //co_opts.paths.strings = (char **)entries;
    //co_opts.paths.count = 1;
    git_object* target;
    if (0 > git_revparse_single(&target, repo, "HEAD")) {
        target = NULL;
        cout << "git_revparse_single fail, reset_commitid:HEAD" << endl;
    }
    error = git_checkout_tree(repo, NULL, &co_opts);
